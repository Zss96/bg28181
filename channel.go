...
func (channel *Channel) CreateRequst(Method sip.RequestMethod) (req sip.Request) {
	...
	if channel.DeviceID[0:9] != host {
		if channel.Port != 0 {
			...
			host = fmt.Sprintf("%s:%d", deviceIp, 7100)
	...
	d.Info("host=====================", zap.String("host", host))
	fmt.Println("============================host", host)
	//DisplayName: sip.String{Str: d.serverConfig.Serial},
	// fmt.Println("============================host", host)
	regHost := regexp.MustCompile(`:\d+`)
	host = regHost.ReplaceAllString(host, fmt.Sprintf(":%d", 7100))
...
func (channel *Channel) Invite(opt *InviteOptions) (code int, err error) {
	...
	if conf.IsMediaNetworkTCP() {
		...
		if conf.tcpPorts.Valid {
			...
			tcpPort, _ := conf.tcpPorts.GetPort()
			opt.recyclePort = conf.tcpPorts.Recycle
			channel.Debug("Invite", zap.String("protocol", protocol), zap.Any("tcpPorts", tcpPort), zap.Any("recyclePort", opt.recyclePort))
			reusePort = false
		}
	} else {
	...
	if opt.MediaPort == 0 {
		// channel.Debug("MediaPort", zap.Any(" conf.MediaPort", conf.MediaPort))
	...
	// channel.Debug("mediaIP========", zap.String("mediaIP", d.mediaIP))
	// newHost := fmt.Sprintf("%s:%d", d.mediaIP, 7100)
	// hostHeader := sip.GenericHeader{
	// 	HeaderName: "Host",
	// 	Contents:   newHost,
	// }
	// invite.AppendHeader(&hostHeader)
